...
N_SHOTS = 5
MIN_IOU_TH = 0.3
PRED_FILE_HEADER = ["filename","start_time","end_time"]
POS_VALUE = 'POS'
UNK_VALUE = 'UNK'
...
def remove_shots_from_ref(ref_df, number_shots=5):
    ...
    events_to_drop = ref_df.index[ref_df['end_time'] <= ref_df.iloc[ref_n_shot_index]['end_time']].tolist()
...
def build_matrix_from_selected_rows(data_frame, selected_indexes_list ):
    ...
    for n, idx in enumerate(selected_indexes_list):
        matrix_data[0, n] = data_frame.loc[idx].start_time # start time for event n
        matrix_data[1, n] = data_frame.loc[idx].end_time
    return matrix_data
...
def compute_tp_fp_fn(pred_events_df, ref_events_df):
    ...
    pred_events_df = pred_events_df.sort_values(by='start_time', axis=0, ascending=True)
...
def evaluate(pred_file_path, ref_file_path, team_name, dataset, savepath, metadata=[]):
    ...
    pred_events_by_audiofile = dict(tuple(pred_csv.groupby('filename')))
    ...
    for audiofilename in list(pred_events_by_audiofile.keys()):
        ...
        try:     
            ...
            ref_events_this_audiofile_all = pd.read_csv(os.path.join(ref_file_path, inv_gt_file_structure[audiofilename], audiofilename[0:-4]+'.csv'), dtype={'start_time':np.float64, 'end_time': np.float64})
            # sort events by starttime:
            ref_events_this_audiofile_all = ref_events_this_audiofile_all.sort_values(by='start_time', axis=0, ascending=True)
    ...
    for audiofilename in list_all_audiofiles:
        if audiofilename not in counts_per_audiofile.keys():
            ...
            ref_events_this_audiofile = ref_events_this_audiofile.sort_values(by='start_time', axis=0, ascending=True)
