...
def time_2_frame(df,fps):
    ...
    df.loc[:,'start_time'] = df['start_time'] - 0.025
    df.loc[:,'end_time'] = df['end_time'] + 0.025

    'Converting time to frames'

    start_time = [int(np.floor(start * fps)) for start in df['start_time']]

    end_time = [int(np.floor(end * fps)) for end in df['end_time']]
    ...
    if mode == 'train':
        ...
        for file in all_csv_files:
            try :
                ...
                print("supression de ce fichier")
                pass
        ...
        print(conf.path.eval_dir)

        all_csv_files = [file
                         for path_dir, subdir, files in os.walk(meta_path)
                         for file in glob(os.path.join(path_dir, extension))]
        print("all csv file :", all_csv_files)
